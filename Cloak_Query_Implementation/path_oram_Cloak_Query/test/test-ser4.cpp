...
void writeTimingMetric(const timingDetails& details, bool truncate = false) {
    ...
    std::ofstream ofs;
    if (truncate) {
		ofs.open("../backup_ser4/timing_metrics.txt", std::ios::out | std::ios::trunc);
	} else {
        ofs.open("../backup_ser4/timing_metrics.txt", std::ios::out | std::ios::app);
	}
...
namespace CloakQueryPathORAM
{
    class ORAMTestSQL : public ::testing::Test
	{
		...
		std::unique_ptr<ORAM> loadORAMAndShares() {
			...
			auto [storage, map, stash, oram] = initializeFromBackup(commonSecretShareSize, 3);
    ...
    TEST_F(ORAMTestSQL, PutContainerServerORAM4)
	{
		...
		auto [storage, map, stash, oram] = initialize(secretShares.size(), 3);
		...
		auto end = std::chrono::high_resolution_clock::now();
	 	...
	 	std::cout << "Time to populate ORAM for server 4" << ": " << duration_ms << " ms" << std::endl;
		...
		callSyncCache(oram);
		backupGenerator(storage, map, stash, oram, 3);
				// Storing the secret share size since it is also used in the backupGenerator
		std::ofstream sizeFile("../backup_ser4/common_secret_share_size.txt");
		if (sizeFile.is_open()) {
    ...
    TEST_F(ORAMTestSQL, GetContainerSingleORAM4)
    {
		...
		details.testName = "GetContainerServerORAM4";
