...
void writeAvgResult(const std::string& resultDir, const std::string& label, int64_t sum, int64_t totalTuples, double avg) {
    std::lock_guard<std::mutex> lock(g_results_mutex);
    std::error_code ec;
    std::filesystem::create_directories(resultDir, ec);

    const std::string perDirPath = resultDir + "/avg_results.txt";
    const std::string combinedPath =
        (std::filesystem::path(resultDir).parent_path() / "avg_results.txt").string();

    auto ensureHeader = [&](const std::string& path) {
        if (g_reset_paths.insert(path).second) {
            std::ofstream reset(path, std::ios::trunc);
            reset << "AVG Reconstruction Results\n";
        }
    };
    ...
    auto appendLine = [&](const std::string& path) {
        ...
        //out << label << " | " << sum << " | " << static_cast<long long>(std::llround(avg)) << "\n";
        out << "Query Result for: " << label << "\n";
        out << "Total Sum: " << sum << "\n";
        out << "Total Tuples: " << totalTuples << "\n";
        out << "Average: " << avg << "\n\n";
    };
...
TEST_P(AvgAggregationTest, ReconstructAvgWithShamirParser) {
    ...
    std::cout << "Actual AVG (Lagrange interpolation): " << actual_sum << std::endl;
    ...
    double avg = (static_cast<double>(actual_sum) / totalTuples);
    std::cout << "Reconstructed average: " << avg << std::endl;
    ...
    writeAvgResult(resultDir, label, actual_sum, totalTuples, avg);
}   
