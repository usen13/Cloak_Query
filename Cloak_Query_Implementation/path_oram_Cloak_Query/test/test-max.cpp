...
void writeMaxResult(const std::string& resultDir, const std::string& label, double maxValue) {
    std::lock_guard<std::mutex> lock(g_results_mutex);
    std::error_code ec;
    std::filesystem::create_directories(resultDir, ec);

    const std::string perDirPath = resultDir + "/max_results.txt";
    const std::string combinedPath =
        (std::filesystem::path(resultDir).parent_path() / "max_results.txt").string();

    auto ensureHeader = [&](const std::string& path) {
        if (g_reset_paths.insert(path).second) {
            std::ofstream reset(path, std::ios::trunc);
            reset << "MAX Reconstruction Results\n";
        }
    };
...
TEST_P(MaxAggregationTest, ReconstructMAXWithShamirParser) {
    ...
    writeMaxResult(resultDir, label, maxValue);
}
