...
void writeTimingMetric(const timingDetails& details, bool truncate = false) {
    ...
    std::ofstream ofs;
    if (truncate) {
		ofs.open("../backup_ser1/timing_metrics.txt", std::ios::out | std::ios::trunc);
	} else {
        ofs.open("../backup_ser1/timing_metrics.txt", std::ios::out | std::ios::app);
	}
...
namespace CloakQueryPathORAM
{
    class ORAMTestSQL : public ::testing::Test
	{
		...
		initialize(size_t secretSharesSize, int serverIndex = 0)
		{
			...
			std::string backupDir = "../backup_ser1";
		...
		std::unique_ptr<ORAM> loadORAMAndShares() {
			...
			std::string usedBlockIDsFile = "../backup_ser1/used_block_ids_server_0.bin";
			...
			oram->loadMacMap("../backup_ser1/mac_map_server_0.bin");
    ...
    TEST_F(ORAMTestSQL, PutContainerServerORAM1)
	{
		...
		std::ofstream sizeFile("../backup_ser1/common_secret_share_size.txt");
		if (sizeFile.is_open()) {
