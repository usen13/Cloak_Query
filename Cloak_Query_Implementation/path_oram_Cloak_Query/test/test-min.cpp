...
std::unordered_set<std::string> g_reset_paths;

// Recreate the output file once per process, then append entries
void writeMinResult(const std::string& resultDir, const std::string& label, double minValue) {
    std::lock_guard<std::mutex> lock(g_results_mutex);
    std::error_code ec;
    std::filesystem::create_directories(resultDir, ec);

    const std::string perDirPath = resultDir + "/min_results.txt";
    const std::string combinedPath =
        (std::filesystem::path(resultDir).parent_path() / "min_results.txt").string();

    auto ensureHeader = [&](const std::string& path) {
        if (g_reset_paths.insert(path).second) {
            std::ofstream reset(path, std::ios::trunc);
            reset << "MIN Reconstruction Results\n";
        }
    };

    ensureHeader(perDirPath);
    ensureHeader(combinedPath);

    auto appendLine = [&](const std::string& path) {
        std::ofstream out(path, std::ios::app);
        out << "Query Result for: " << label << "\n";
        out << "Min Value: " << minValue << "\n";
    };

    // Write to per-folder file and to a combined file
    appendLine(perDirPath);
...
double getMinFromItems(const std::vector<LineItem>& items, const std::string& attrName) {
    double minValue = std::numeric_limits<double>::max();
    for (const auto& item : items) {
        if (attrName == "EXTENDEDPRICE") {
            if (item.L_EXTENDEDPRICE < minValue) minValue = item.L_EXTENDEDPRICE;
        } else if (attrName == "QUANTITY") {
            if (item.L_QUANTITY < minValue) minValue = item.L_QUANTITY;
        } else if (attrName == "DISCOUNT") {
            if (item.L_DISCOUNT < minValue) minValue = item.L_DISCOUNT;
        } else if (attrName == "TAX") {
            if (item.L_TAX < minValue) minValue = item.L_TAX;
        }
...
protected:
    ...
    int threshold = 3;
    int totalTuples = 0;
    std::string attrName; // Attribute name to be used for min aggregation
    size_t sumIdx;
...
TEST_P(MinAggregationTest, ReconstructMINWithShamirParser) {
    ...
    for (size_t i = 0; i < tempShares[0].size(); ++i) {
        ...
        // Usage:
        double minValue = getMinFromItems(reconstructedItems, attrName);
        std::cout << "Min value for " << attrName << ": " << minValue << std::endl;
        std::cout << "Reconstructed tuples written to: " << filename << std::endl;

    // Write result to the related Query_Result subfolder file
    auto [jsonPath, resultDir] = GetParam();
    const std::string label = std::filesystem::path(jsonPath).filename().string();
    writeMinResult(resultDir, label, minValue);
}
