...
size_t loadCommonSecretShareSize() {
    std::ifstream sizeFile("../backup_ser6/common_secret_share_size.txt");
    size_t size = 0;
    if (sizeFile.is_open()) {
        sizeFile >> size;
        sizeFile.close();
...
void writeTimingMetric(const timingDetails& details, bool truncate = false) {
    ...
    std::ofstream ofs;
    if (truncate) {
		ofs.open("../backup_ser6/timing_metrics.txt", std::ios::out | std::ios::trunc);
	} else {
        ofs.open("../backup_ser6/timing_metrics.txt", std::ios::out | std::ios::app);
	}
...
namespace CloakQueryPathORAM
{
    class ORAMTestSQL : public ::testing::Test
	{
		...
		initialize(size_t secretSharesSize, int serverIndex = 0)
		{
			...
			std::string backupDir = "../backup_ser6";
		...
		std::unique_ptr<ORAM> loadORAMAndShares() {
			...
			auto [storage, map, stash, oram] = initializeFromBackup(commonSecretShareSize, 5);
			std::string usedBlockIDsFile = "../backup_ser6/used_block_ids_server_5.bin";
			...
			oram->loadMacMap("../backup_ser6/mac_map_server_5.bin");
    ...
    TEST_F(ORAMTestSQL, PutContainerServerORAM6)
	{
		...
		auto [storage, map, stash, oram] = initialize(secretShares.size(), 5);
		...
		auto end = std::chrono::high_resolution_clock::now();
	 	...
	 	std::cout << "Time to populate ORAM for server 6" << ": " << duration_ms << " ms" << std::endl;
		...
		callSyncCache(oram);
		backupGenerator(storage, map, stash, oram, 5);
				// Storing the secret share size since it is also used in the backupGenerator
		std::ofstream sizeFile("../backup_ser6/common_secret_share_size.txt");
		if (sizeFile.is_open()) {
    ...
    TEST_F(ORAMTestSQL, GetContainerSingleORAM6)
    {
        timingDetails details = {};
		details.testName = "GetContainerServerORAM6";
        ...
        auto [storage, map, stash, oram] = initializeFromBackup(commonSecretShareSize, 5); // Initialize from backup for server 0
        std::string usedBlockIDsFile = "../backup_ser6/used_block_ids_server_5.bin";
	...
	TEST_F(ORAMTestSQL, SQLCountORQuery) {
		...
		for (const auto& filter : j["filters"]) {
			if (filter.contains("shareID") && filter["shareID"].contains("id_5")) {
				filter_ids.push_back(filter["shareID"]["id_5"].get<int64_t>());
			}
	...
	TEST_F(ORAMTestSQL, SQLSUMORQuery) {
		...
		std::ofstream outFile(resultDir + "/server_6.txt");
